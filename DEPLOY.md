...
## ❓ Может ли Claude залить сам?
**Без доступа к хостингу — нет.** Заливка происходит внутри панели SuperHosting (cPanel), куда нужен ЛОГИН Анатолия.

Возможны два варианта:
- **A. Анатолий сам (≈5 минут, проще и безопаснее):** создать поддомен в cPanel → File Manager → Upload `eurosfera-deploy.zip` → Extract. Шаги ниже.
- **B. Claude заливает файлы (если дан доступ):** Анатолий создаёт поддомен + почту + крон в cPanel (это клики в панели, по FTP не делается), а **доступ FTP/SSH кладёт в Связку ключей** (НЕ в чат — см. `reference_claude_secure_toolkit`). Тогда Claude через `lftp`/`curl`/`scp` зальёт файлы в папку поддомена и проверит.

> Итог: поддомен/почту/крон в панели делает Анатолий в любом случае; **сам аплоад файлов** Claude может взять на себя при доступе по FTP/SSH. Скажи, по какому пути идём — подготовлю.
...
---
**Контакты на сайте:** sale@euro.s.bg (IT: it@euro.s.bg) · Telegram @gagauz13 · +359 89 209 84 60 и +359 89 209 84 58

**Хостинг:** SuperHosting.BG, панель cPanel (jupiter) — вход через odin.superhosting.bg:2083 (логин у Анатолия; ссылки с `cpsessXXXX` — одноразовые сессии, не сохранять).
